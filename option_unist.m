image1 = rescale(imread("images/panorama/unist_right.jpg"));
image2 = rescale(imread("images/panorama/unist_left.jpg"));
...
%%
[features1, eigen1] = eigen_vectors(image1, 0.2022);
[row1,col1] = find(features1);
feat_vec1 = [row1 col1];
[features2, eigen2] = eigen_vectors(image2, 0.20712);
...
hold off
% imwrite(getframe(gcf).cdata, 'task5/pano1.jpg')
%%
imshow(image2)
hold on
for i = 1:size(col2)
...
hold off
% imwrite(getframe(gcf).cdata, 'task5/pano2.jpg')
...
%%
ssd_im = rescale(ssd);
ssd_im = rescale(imresize(ssd, 20,'nearest'));
imshow(ssd_im);

%%
col_filt_ssd = ssd(:,min(ssd,[],1)<10);
row2 = row2(min(ssd,[],1)<10,:);
col2 = col2(min(ssd,[],1)<10,:);
